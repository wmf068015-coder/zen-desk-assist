...
import { sessions as allSessions, type Session, type SessionStatus, CHANNEL_LABELS, TAG_LABELS } from "@/lib/mock-data";
...
export function SessionList({ activeId, onSelect, maxCapacity, onCapacityChange }: Props) {
  ...
  }, [filter, query, channelFilter, tagFilter, transferredOnly]);

  const activeCount = allSessions.filter((s) => s.status === "human").length;
  ...
  return (
    <div className="flex h-full w-[340px] flex-col border-r bg-card">
      <div className="space-y-3 border-b p-4">
        ...
        {showAdvanced && (
          <div className="space-y-2 rounded-lg bg-muted/50 p-3">
            <div className="flex items-center gap-2">
              ...
              <select value={channelFilter} onChange={(e) => setChannelFilter(e.target.value)} className="h-7 flex-1 rounded border bg-background px-2 text-xs">
            ...
            <div className="flex items-center gap-2">
              ...
              <select value={tagFilter} onChange={(e) => setTagFilter(e.target.value)} className="h-7 flex-1 rounded border bg-background px-2 text-xs">
                <option value="all">全部标签</option>
                {Object.entries(TAG_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
              </select>
            </div>
            <label className="flex items-center gap-2 text-xs">
              <input type="checkbox" checked={transferredOnly} onChange={(e) => setTransferredOnly(e.target.checked)} className="accent-primary" />
              仅显示已转人工
            </label>
          </div>
        )}
        <div className="flex items-center justify-between rounded-lg border bg-accent/30 px-3 py-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs">接待中 <b className="text-foreground">{activeCount}</b> / {maxCapacity}</span>
          </div>
          <input
            type="number"
            min={1}
      ...
      <div className="scrollbar-thin flex-1 overflow-y-auto">
        {filtered.map((s) => (
          <SessionItem key={s.id} session={s} active={s.id === activeId} onClick={() => onSelect(s.id)} />
        ))}
        {filtered.length === 0 && (
  ...
  return (
    ...
    >
      <div className="relative shrink-0">
        <img src={session.customer.avatar} alt={session.customer.name} className="h-10 w-10 rounded-full bg-muted" />
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex items-center gap-1.5">
          ...
          {session.transferred && (
            <span className="inline-flex items-center gap-0.5 rounded bg-primary/10 px-1.5 py-0.5 text-[10px] text-primary">
              <ArrowRightLeft className="h-2.5 w-2.5" />转人工
            </span>
          )}
        </div>
        ...
        <div className="mt-1.5 flex items-center justify-between">
          <div className="flex gap-1">
            {session.tags.slice(0, 2).map((t) => <TagBadge key={t} tag={t} />)}
          </div>
          {session.unread > 0 && (
